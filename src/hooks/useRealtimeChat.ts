      ...
      .on(
        ...
        (payload) => {
          ...
          if (eventType === "INSERT" && newRecord) {
            ...
            if (notify) {
              notify("Nova Conversa", `${newRecord.client_name} iniciou um chat`, "new", newRecord.id, newRecord.assigned_to);
            }
            ...
            if (newRecord.assigned_to === userId && oldRecord?.assigned_to !== userId && notify) {
              notify("Conversa Atribuída", `Você agora é o responsável por ${newRecord.client_name}`, "assigned", newRecord.id, newRecord.assigned_to);
            }
            storeRef.current.addDbConversation({
              id: newRecord.id,
              clientName: newRecord.client_name,
